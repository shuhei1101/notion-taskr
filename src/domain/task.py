...
class Task:
    ...
    def toggle_is_updated(self, update_message: str):
        '''is_updatedをトグルする'''
    ...
    def update_man_hours_label(self, man_hours_label: 'ManHoursLabel'):
        ...
        if self.name.man_hours_label != man_hours_label:
            self.toggle_is_updated(f'工数ラベル: {self.name.man_hours_label} -> {man_hours_label}')
    ...
    def update_id_label(self, label: 'IdLabel'):
        ...
        if self.name.id_label != label:
            self.toggle_is_updated(f'IDラベル: {self.name.id_label} -> {label}')
    ...
    def update_parent_id_label(self, parent_id_label: 'ParentIdLabel'):
        ...
        if self.name.parent_id_label != parent_id_label:
            self.toggle_is_updated(f'親IDラベル: {self.name.parent_id_label} -> {parent_id_label}')
            self.name.parent_id_label = parent_id_label 

    def update_status(self, status: Status):
        '''ステータスを更新し、is_updatedをTrueにする'''
        if self.status != status:
            self.toggle_is_updated(f'ステータス: {self.status} -> {status}')
            self.status = status

    def update_name(self, name: TaskName):
        '''タスク名を更新し、is_updatedをTrueにする'''
        if self.name != name:
            self.toggle_is_updated(f'タスク名: {self.name} -> {name}')
            self.name = name
